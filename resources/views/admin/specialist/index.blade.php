...
@section('content')
    <div class="title-header">
        <h3>Specjalisci</h3>
        <div class="actions">
            <a href="{{ route('specjalisci.create') }}" class="btn btn-primary"><i class="lni lni-plus"></i> Dodaj nowego
                specjaliste</a>
        </div>
    </div>
        ...
        <table class="table ">
            ...
            <tbody>
                @foreach ($specialists as $key => $specialist)
                    <tr class="{{ $specialist->status === 0 ? 'inactive-tr' : 'active-tr' }}">
                        <td scope="
                            row">{{ $specialist->id }}</td>
                        <td>
                            <div style="display: flex; align-items: center">
                                <img src="{{ Storage::url('files/shares/specjalisci/' . $specialist->photo) }}"
                                    width="50px">
                        ...
                        <td><select class="form-control change" data-attribute="status" data-id="{{ $specialist->id }}"
                                data-slug="{{ $specialist->slug }}">
                                <option data-status="1" {{ $specialist->status === 1 ? 'selected' : '' }}>Widoczny
                                </option>
                                <option data-status="0" {{ $specialist->status === 0 ? 'selected' : '' }}>Ukryty</option>
                            </select>
                        </td>
                        <td style="text-align: right">
                            <a href="{{ route('specjalisci.edit', $specialist->id) }}"><i
                        ...
                        <td>
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                <a class="dropdown-item"
                                    href="{{ route('specjalisci.show', $specialist->id) }}">Zobacz</a>
                                @csrf
                                <a class="dropdown-item" href="#" data-type="delete" data-id="{{ $specialist->id }}"
                                    data-slug="{{ $specialist->id }}">Usuń</a>
                            </div>
                        </td>
                    </tr>
                @endforeach
